...
무엇을 보여 주나
  llm.claim 뷰의 청구 건을 rules/HIRA_RULES.md 와 대조해, 건마다
  어떤 규정이 걸리는지 · 진료일이 시행일 이후인지 · 금액이 본인부담률과 맞는지 보여 준다.

왜 AI 가 아니라 여기서 계산하나
  이 판정들은 전부 결정적이다 — 날짜 비교와 산술이다. 지어낼 여지가 없어야 하고,
  같은 입력이면 늘 같은 화면이 나와야 한다. AI 의 판단은 이슈 댓글에 남고,
...
COLUMNS = [
    "patient_token", "treatment_date", "visit_type", "department_name",
...
def judge(claim: dict, rules: list[dict]) -> dict:
    ...
    if any(f["level"] == "danger" for f in flags):
        ...
        verdict = "규정 없음"
    return {"verdict": verdict, "rules": hits, "flags": flags}


def build(claims: list[dict], rules: list[dict]) -> dict:
    days: dict[str, list] = {}
    for c in claims:
...
def self_check() -> int:
    ...
    assert all(r["effective"] for r in rules), \
        f"시행일 없는 규정: {[r['id'] for r in rules if not r['effective']]}"
