...
3. `.hira-fetch/` 에 공고 목록이 이미 받아져 있다. 인터넷에 직접 접근하지 않는다 —
   에이전트 방화벽은 심평원·복지부에 닿지 못하고, 조용히 실패한다.
   여기 있는 것만 근거로 쓴다.

   - `.hira-fetch/manifest.json` — 출처별 상태(`ok` / `unreachable` / `http_*` / `parse_failed`)
   - `.hira-fetch/hira-notice.md` — 심평원 공지사항 (공고일 · 제목 · 링크)
   - `.hira-fetch/mohw-notice.md` — 보건복지부 고시·훈령·예규

   **먼저 `manifest.json` 을 본다.** 모든 출처가 `ok` 가 아니면 9번으로 간다.
   `parse_failed` 는 "변경 없음"이 아니라 "페이지 구조가 바뀌었다"는 뜻이다.

   목록에는 공고일만 있다. **시행일은 상세 페이지에 있다** — 청구에 닿을 것 같은
   공고는 링크를 열어 시행일과 적용 진료분을 확인한다. 열 수 없으면 그 사실을 적는다.
...
9. `manifest.json` 의 출처 중 하나라도 `ok` 가 아니면 — 파일을 고치지 말고
