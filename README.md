...
|---|---|---|
| 저장소 (git · 이슈 · PR) | **없음** | — |
| 실행 로그 | **없음** — 환자 ID 를 `env:` 로 받지 않습니다 | — |
| `billing-intake` 실행의 입력값 | 환자 ID | **30분** (`sweep-runs` 가 지웁니다) |
| `billing-review` 실행의 입력값 | 비식별 진료행 JSON | 7일 |
| 실행 Artifact `billing-report` | 판단 보고서 | 7일 |

입력값이 남는 것은 페이지가 정적이어서가 아닙니다. `workflow_dispatch` 를 부르는 쪽이
정적 페이지든 동적 페이지든 `gh` CLI 든 Actions 폼이든, 깃헙은 "실행할 때 무엇을
넣었나" 를 그 실행에 붙여 둡니다. 보내는 값 자체를 바꾸지 않는 한 피할 수 없습니다.

동적 서버를 두면 그 서버가 ID 를 가명으로 바꿔 보낼 수 있어 깃헙 쪽에는 가명만 남습니다.
대신 그 서버가 새 기록 장소가 되고(요청 로그·ID 매핑), 매핑을 가지려면 DB 를 깃헙 밖에
따로 띄워야 합니다. 기록이 사라지는 게 아니라 관리할 곳이 하나 더 느는 것입니다.

그래서 없애는 대신 **수명을 줬습니다.** `sweep-runs` 가 30분마다 돌며 지난 요청 실행을
지웁니다. 30분인 이유는 화면이 진행을 지켜보는 동안 실행이 사라지면 안 되기 때문입니다.

`env:` 로 받지 않는 이유도 같은 자리에서 나왔습니다. 러너는 스텝을 시작할 때 env 목록을
로그에 먼저 찍는데, 그건 `::add-mask::` 가 걸리기 전입니다 — `INPUT_PATIENT_ID: P00013`
한 줄이 그대로 남았습니다. 지금은 이벤트 파일에서 직접 읽어 로그에 나올 자리가 없습니다.
